...
{
    public class ElaboraDividendiFineco
    {
        static HttpClient httpClient = new HttpClient();

        const string SimboliFileName = "SimboliDescrizioni.xlsx";
        const string AggregatoFileName = "FinecoAggregato.xlsx";

        public async Task<string> ElaboraDividendi(string finecoFolder)
        {
            string outputFile = Path.Combine(finecoFolder, AggregatoFileName);
            ...
            var titoli = CaricaTitoli(finecoFolder);

            foreach (var file in Directory.EnumerateFiles(finecoFolder, "*.xls?", SearchOption.AllDirectories))
            {
        ...
        Dividendo[] CalcolaDividendiNetti(FinecoRow[] importi)
        {
        ...
        void AddHeaderDividendiToSheet(IXLWorksheet wb)
        {
            var header = wb.Row(1);
        ...
        void AppendRowDividendiToSheet(IXLWorksheet sheet, int rowIndex, Dividendo importo)
        {
        ...
        void AddHeaderTotaleToSheet(IXLWorksheet wb)
        {
            var header = wb.Row(1);
        ...
        void AppendRowTotaleToSheet(IXLWorksheet sheet, int rowIndex, FinecoRow importo)
        {
            var row = sheet.Row(rowIndex);
        ...
        async Task<Rates> GetCambioEUR_USDInData(HttpClient client, DateTime data)
        {
        ...
        Titolo[] CaricaTitoli(string folder)
        {
