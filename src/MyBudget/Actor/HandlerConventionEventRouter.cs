namespace CommonDomain.Core
{
	...
	public class HandlerConventionEventRouter : IRouteEvents
	{
	    ...
	    public HandlerConventionEventRouter() : this(true)
	    {
	    }

	    public HandlerConventionEventRouter(bool throwOnApplyNotFound)
	    {
	        this.throwOnApplyNotFound = throwOnApplyNotFound;
	    }

        public HandlerConventionEventRouter(bool throwOnApplyNotFound, IAggregate aggregate)
		...
		public virtual void Register(IAggregate aggregate)
		{
            ...
            var applyMethods = aggregate.GetType()
				...
				.Where(m => m.Name == "Handle" && m.GetParameters().Length == 1 && m.ReturnParameter.ParameterType == typeof(void))
				.Select(m => new
				{
					Method = m,
